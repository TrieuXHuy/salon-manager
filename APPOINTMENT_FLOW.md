...
## 7. Nhắc lịch qua email

Hệ thống có 2 kiểu gửi email:

- Gửi thủ công: admin hoặc nhân viên chọn lịch `Đã giữ chỗ` rồi bấm `Nhắc lịch`.
- Gửi tự động: hệ thống tự quét lịch bằng cron job.

Luồng tự động:

- Scheduler được bật bằng `@EnableScheduling`.
- Job chạy mỗi 30 phút, đúng phút `00` và `30`.
- Cron hiện tại: `0 0/30 * * * *`.
- Mỗi lần chạy, hệ thống lấy thời gian hiện tại của máy.
- Sau đó quét các lịch có trạng thái `Đã giữ chỗ` nằm trong 30 phút sắp tới.
- Nếu khách có email, hệ thống gửi email nhắc lịch bằng `JavaMailSender`.
- Nếu một email gửi lỗi, hệ thống chỉ ghi log lỗi lịch đó và tiếp tục gửi các lịch còn lại.

Ví dụ:

```text
11:00 job chạy
-> quét lịch từ 11:00 đến 11:30
-> có lịch Đã giữ chỗ thì gửi email nhắc

11:30 job chạy
-> quét lịch từ 11:30 đến 12:00
-> có lịch Đã giữ chỗ thì gửi email nhắc
```

Điều kiện để gửi email thật:

- Project dùng dependency `spring-boot-starter-mail`.
- Service gửi mail dùng `JavaMailSender`.
- Cần cấu hình SMTP trong `application.yaml` hoặc biến môi trường:
...
- `username` là Gmail gửi đi.
- `password` nên là Gmail App Password, không dùng mật khẩu đăng nhập Gmail thường.
- Không nên commit email/password thật lên Git.

## 8. Quyền thao tác theo vai trò
...
### Case 7: Gửi nhắc lịch thủ công

1. Tạo một lịch ở trạng thái `Đã giữ chỗ`.
2. Đảm bảo khách hàng có email.
3. Đăng nhập admin hoặc nhân viên.
4. Chọn lịch đó.
5. Bấm `Nhắc lịch`.
6. Kiểm tra email được gửi tới khách.

### Case 8: Gửi nhắc lịch tự động

1. Cấu hình email SMTP đúng.
2. Tạo một lịch `Đã giữ chỗ` có thời gian nằm trong 30 phút sắp tới.
3. Chạy app và chờ tới mốc phút `00` hoặc `30`.
4. Kiểm tra log có dòng gửi email thành công.
5. Kiểm tra hộp thư khách hàng nhận được email nhắc lịch.
